...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "affine-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("AFFINE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("AFFINE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "affine-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/affine-pp-cli/feedback.jsonl.
When ` + "`AFFINE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`AFFINE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  affine-pp-cli feedback list
  affine-pp-cli feedback list --limit 5
  affine-pp-cli feedback list --json`,
