...
type Config struct {
	...
	AffineToken   string            `toml:"token"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://qtz-affine.fly.dev",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("AFFINE_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "affine-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("AFFINE_TOKEN"); v != "" {
		cfg.AffineToken = v
		cfg.markEnvOverride("AffineToken")
		cfg.AuthSource = "env:AFFINE_TOKEN"
	}
	...
	if cfg.AuthSource == "" && cfg.AffineToken != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("AFFINE_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.AffineToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:AFFINE_TOKEN"
		}
		return "Bearer " + c.AffineToken
	}
	if c.AccessToken != "" {
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.AffineToken = ""
	delete(c.envOverrides, "AffineToken")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["AffineToken"] {
			out.AffineToken = c.fileConfig.AffineToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "AffineToken":
		c.fileConfig.AffineToken = c.AffineToken
	}
}
